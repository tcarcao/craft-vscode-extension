...
# Craft VS Code Extension Release Script
...
if [ ! -f "package.json" ] || [ ! -d ".vscode" ]; then
    echo -e "${RED}❌ Must be run from the craft-vscode-extension root directory${NC}"
    exit 1
fi
...
if [ -z "$VERSION" ]; then
    ...
    echo "Examples:"
    echo "  $0 0.1.0    # Create v0.1.0 release"
...
echo "  - Build and bundle the extension"
echo "  - Package as .vsix file"
echo "  - Create GitHub release with .vsix attachment"
echo "  - Commit version bump back to main branch"
echo ""
echo -e "${BLUE}🔗 Monitor progress:${NC} https://github.com/tcarcao/craft-vscode-extension/actions"